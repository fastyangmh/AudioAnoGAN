...
def create_model(project_parameters):
    model = UnsupervisedModel(
        ...
        _lambda=project_parameters._lambda,
...
class Generator(nn.Module):
    def __init__(self, input_height, in_chans, generator_feature_dim,
                 latent_dim) -> None:
        super().__init__()
        self.generator = Decoder(input_height=input_height,
                                 in_chans=in_chans,
...
class UnsupervisedModel(BaseModel):
    def __init__(self, optimizers_config, lr, lr_schedulers_config, in_chans,
                 ...
                 discriminator_feature_dim, _lambda, adversarial_weight,
    ...
    def configure_optimizers(self):
        ...
        optimizers_e = self.parse_optimizers(params=self.encoder.parameters())
        if self.lr_schedulers_config is not None:
            ...
            lr_schedulers_e = self.parse_lr_schedulers(optimizers=optimizers_e)
            return [optimizers_g[0], optimizers_d[0], optimizers_e[0]], [
                lr_schedulers_g[0], lr_schedulers_d[0], lr_schedulers_e[0]
            ]
        else:
            return [optimizers_g[0], optimizers_d[0], optimizers_e[0]]
    ...
    def forward(self, x):
        z = self.encoder(x)
        x_hat = self.generator(z)
        _, feat_x = self.discriminator(x)
        _, feat_x_hat = self.discriminator(x_hat)
        residual_loss = F.l1_loss(input=x_hat, target=x,
                                  reduction='none').mean(dim=(1, 2, 3))
        discrimination_loss = F.l1_loss(input=feat_x_hat,
                                        target=feat_x,
                                        reduction='none').mean(dim=(1, 2, 3))
        loss = (1 - self._lambda
                ) * residual_loss + self._lambda * discrimination_loss
        return loss, x_hat
    ...
    def shared_step(self, batch):
        ...
        return x, x_hat, prob_x, feat_x, prob_x_hat, feat_x_hat

    def training_step(self, batch, batch_idx, optimizer_idx):
        x, x_hat, prob_x, feat_x, prob_x_hat, feat_x_hat = self.shared_step(
            batch=batch)
        ...
        if optimizer_idx == 1:  #discriminator
            ...
            self.log('train_loss_discriminator',
                     ...
                     logger=True)
            return d_loss
        if optimizer_idx == 2:  #encoder
            #TODO: needs to check the backpropagation
            z_hat = self.encoder(x)
            # lock the weight of generator and discriminator
            self.generator.eval()
            self.discriminator.eval()
            x_hat = self.generator(z_hat)
            _, feat_x = self.discriminator(x)
            _, feat_x_hat = self.discriminator(x_hat)
            residual_loss = self.l1_loss(x_hat, x)
            discrimination_loss = self.l1_loss(feat_x_hat, feat_x)
            e_loss = (1 - self._lambda
                      ) * residual_loss + self._lambda * discrimination_loss
            e_loss *= self.encoding_weight
            # unlock the weight of generator and discriminator
            self.generator.train()
            self.discriminator.train()
            self.log('train_loss_encoder',
                     e_loss,
                     on_step=True,
                     on_epoch=True,
                     prog_bar=True,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        self.log('val_loss_encoder', e_loss, prog_bar=True)
